...
Used to upack DynamoDB backup files in the SWA bucket. The script was executed on an EC2 instance with enough storage to unpack the backup files (±120 Gb). 
...
* run `rclone sync google:inactive-usage-data/ gcs/ --progress --include 'daily_usage*'` to download the files
* run the script
* copy the unpacked files to the final destination `rclone sync unpacked/p4_hour_data/ s3:groengegeven-swa-v2-export/gcs/p4_hour_2024/ --progress`
...
* 'p4_hour_data' for 2024 is 40k files / 390 Gb, sync took x hours, parsing x hours and uploading 2hours.
...
* update daily usage data: combine cold storage (until 40 days ago) with recent data
  ...
  * extract from Supabase using `python scripts/runextraction.py`
  * sync to S3 `rclone sync extracted/daily_usage_data/ s3:slimwonen-analysis-data/gcs/daily_usage/ --progress`